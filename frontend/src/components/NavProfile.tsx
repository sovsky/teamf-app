...
const {handleLogout} = useLogout();
    ...
    {NavProfileItems.map(({label, type, onClick})=>{
        return(
           <>
                    <DropdownMenuItem className={`${type ==="link" ?"text-slate-800 cursor-pointer hover:bg-blue-100" :"text-rose-700 hover:text-rose-600 cursor-pointer" }`} onClick={onClick} >{label}</DropdownMenuItem>
